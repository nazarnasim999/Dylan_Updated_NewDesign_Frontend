...
const MatchingLeadsVendor = () => {
    ...
    const { get_matching_customer_status,
    ...
    useEffect(() => {
        ...
        // }
    }, [dispatch, vendor_login_data?.selected_queries, vendor_data?.selected_queries]);
    return (
        <div className="classname-510">
        {get_matching_customer_data?.length > 0 ? get_matching_customer_data?.map((elem, ind) => {
            return elem?.jobs.map((e, i) => {
                const timeAgo = getTimeAgo(e.jobs?.createdAt);
                return (
                    <Stack key={i}>
                        {!e?.userDetails?.Name ? (
                            <Stack sx={{ fontSize: 12, ml: 2, fontWeight: 'bold' }} className="classname-511">
                                    ...
                                    <Stack p={1} style={{ paddingRight: '10px', borderBottom: '1px solid #01BAF2' }} className=" classname-501">
                                        <Stack onClick={() => sendDataToParent(e, i + 1)} sx={{ cursor: 'pointer' }} flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'}>
                                            <Stack flexDirection={'row'} alignItems={'center'}>
                                                ...
                                                <Stack sx={{ color: '#000000', fontSize: 15, fontWeight: 1000 }} className="classname-513">
                                                    {e?.userDetails?.Name}
                                                </Stack>
                                            </Stack>
                                            <Stack sx={{ backgroundColor: vendor_color, borderRadius: '6px', p: 0.5, fontSize: 9, color: 'white', fontWeight: 'bold' }} className="classname-514">
                                                {timeAgo}
                                            </Stack>
                                        </Stack>
                                        <Stack ml={2} sx={{ cursor: 'pointer' }} onClick={() => sendDataToParent(e, i + 1)} className="classname-515">
                                            <Stack sx={{ color: '#000000', fontSize: 12 }}>
                                                {e?.jobs?.emergency && e?.jobs?.emergency.trim() === 'emergency' && (
                                                    <>
                                                        {'Emergency'}
                                                    </>
                                                )}
                                            </Stack>
                                            <Stack sx={{ color: 'black', fontWeight: "5px", fontSize: 15 }} className="classname-516">
                                            ...
                                            <Stack ml={-0.3} flexDirection={'row'} alignItems={'center'}>
                                                ...
                                                <Stack sx={{ fontSize: 10, color: '#333333' }} className="classname-519">
                                            ...
                                            <Stack ml={-0.3} flexDirection={'row'} alignItems={'center'}>
                                                {e.jobs.availablity_times?.map((e, i) => {
                                                    ...
                                                    const formattedDate = dateObject ? new Intl.DateTimeFormat('en-US', { year: 'numeric', month: 'long', day: 'numeric' }).format(dateObject) : "---";
                                                    return (
                                                        <Stack key={i} ml={0.4} flexDirection={'row'} alignItems={'center'} sx={{ color: '#333333' }} className="classname-520">
                                                            {formattedDate}
                                                        </Stack>
                                                    );
                                                })}
                                        ...
                                        <Stack className='hello-bhai' style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center' }}>
                                            <Stack ml={2} sx={{ cursor: 'pointer' }} style={{ backgroundColor: '#01BAF2', padding: '5px 30px 5px 30px', borderRadius: '5px', color: 'white', fontSize: '20px', fontWeight: 'bold' }} className=" classname-506" onClick={() => sendDataToParent(e, i + 1)}>
                                                Chat Now
                                            </Stack>
                                            <Stack ml={2} sx={{ cursor: 'pointer' }} style={{ backgroundColor: 'white', padding: '5px 10px 5px 10px', borderRadius: '5px', color: 'white', fontSize: '20px', border: '1px solid #002758' }} className="classname-507">
        ...
        }) : <Stack sx={{ textAlign: 'center', alignItems: 'center' }} className="classname-523">No Job Available</Stack>}
    </div>
